...
class Trainer:
    ...
    def view_experience(self):
        for state, action, reward, next_state, done, info in self.memory:
            ...
            state = cv2.putText(state, str(action), (150, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255))
    ...
    def update_epsilon_by_mean_reward_2(self, reward, expectation=4.0):
        ...
        if self.research_mode and self.research_step < 10:
            return
    ...
    def train(self):
        ...
        while self.total_source_reward < self.TARGET_REWARD and current_try < self.TRY_COUNT:
            ...
            self.max_lives = 3
            current_try += 1
            self.local_reward = deque(maxlen=20)
            ...
            if self.total_source_reward > 5000:
                import pickle
                with open('D:\\PROJECTS\\GENERAL\\RESEARCH & CONTEST\\Retro GYM\\best_replay.pkl', 'wb') as handle:
