...
class Trainer:
    def __init__(self):
        ...
        self.AGENT_NAME = 'agent-7_best-sol_update-mean-reward_4'
        ...
        self.research_steps = 50
        self.actions = self.random_action_series()
    ...
    def number_to_act_code(self, number):
        code = np.zeros(7)
        code[number] = 1.0
        return code
    ...
    def change_series(self, series):
        ...
        change_index = random.sample(range(0, leng), int(0.5* leng))
        for ind in change_index:
            new_series[ind]=self.number_to_act_code(random.randint(0,6))
        
        return new_series

    def random_action_series(self):
        actions = np.random.randint(0,7, size=self.research_steps)
        # print(actions)
        # exit()
        return [self.number_to_act_code(a) for a in actions]

    def update_epsilon_by_mean_reward_2(self, reward, expectation=4.0):
        self.local_reward.append(reward)
        self.research_step+=1

        if self.research_mode and self.research_step <= self.research_steps:
            # self.reward -= 10
            return

        self.research_mode = False
        # print('end research mode')
        self.research_step = 0
        ...
        # if np.array(self.local_reward).mean() < expectation:
            ...
            self.reward -= 50
            self.actions = self.change_series(self.actions)
            # print(self.actions)
