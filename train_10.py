...
Подобно trainer_9 используется карта, но только Локальная, т.к. есть подозрение, что полная карта содержит очень мало информации
относительно всего полотна.
И также используется локальный фрейм (т.е. не все что видно на экране, а лишь часть)
"""

from agent_10 import CNQAgent
import numpy as np
...
class Trainer:
    def __init__(self):
        ...
        self.AGENT_NAME = 'agent-10_best-sol_update-mean-reward_5'
        ...
        self.rings = 0
        self.pos_x = 0
        self.map = np.zeros((22, 20), dtype=np.uint8)
        self.prev_pos = (0,0)
        self.kx = 0
    ...
    def new_local_map(self, offset_x):
        from scipy.ndimage.interpolation import shift
        new_map = np.copy(self.map)
        return shift(new_map, offset_x, cval=0)

    def update_map(self):
        source_x = int(self.info['x']/30) 
        x = int(self.info['x']/30) - self.kx
        y = int(self.info['y']/100)
        map_x = self.map.shape[0]
        offset_x = x - (map_x - 1)
        if x >=  map_x or x < 0:
            self.new_local_map(offset_x)
            self.kx += offset_x
    ...
    def do(self):
        ...
        if act_value == 1:
            self.press_buttons([7], self.INERCIA)
        if act_value == 2:
            self.press_buttons([6], self.INERCIA)
        if act_value == 3:
    ...
    def train(self):
        ...
        while self.total_source_reward < self.TARGET_REWARD and current_try < self.TRY_COUNT:
            ...
            self.max_lives = 3
            current_try += 1
            self.local_reward = deque(maxlen=80)
            self.state = self.env.reset()
            self.done = False
            self.memory.clear()
            self.map = np.zeros((22, 20),dtype=np.int8)
