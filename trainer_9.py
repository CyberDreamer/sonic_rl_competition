...
В качестве второго входа используется карта траектории продвижения Соника. 
В тех позициях где Соник появляется несколько раз увеличивается счетчик, что влияет на награду. 
Если агент посещает одни и те же локации вновь и вновь, то это накладывает штраф, а если посещает новые локации, 
то агент получает награду. Гипотеза заключается в том, что новые области (ранее не посещенные) будут видны сети в виде серых областей, 
а много раз посещенные в виде белых. А значит если ты раньше двигался (нажимал вверх) и попадал в серую область и получал награду, то
если сейчас впал в ступор, то двигайся в серую зону. Но понятно что аппроксимировать надо не одну кнопку, а целую комбинацию движения, 
так что может и не сработает. Но опять же, на другом фрейме виден конкретный уровень и нейронка должна понять как именно двигаться, а 
по карте - куда двигаться.
"""

# from agent_7 import CNQAgent
from agent_9 import CNQAgent
import numpy as np
...
class Trainer:
    def __init__(self):
        ...
        self.AGENT_NAME = 'agent-9_best-sol_update-mean-reward_5'
        ...
        self.rings = 0
        self.pos_x = 0
        self.map = np.zeros((188, 20),dtype=np.uint8)
        self.prev_pos = (0,0)
    ...
    def view_frame(self):
        ...
        state = cv2.putText(state, str(round(self.total_source_reward, 1)), (220, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255))
    ...
    def update_epsilon_by_mean_reward_2(self, reward, expectation=4.0):
        ...
        if self.research_mode and self.research_step < 7:
            # self.reward -= 10
            return
    ...
    def update_reward(self, source_reward):
        ...
        # if delta_pos > -100.0 and delta_pos <= 0.0:
        #     source_reward = 0.0
        ...
        # self.reward += self.reward_for_research()
        # self.reward = source_reward
        # self.reward -= 1

    def update_map(self):
        x = int(self.info['x']/60)
        y = int(self.info['y']/100)
        current_pos = (x, y)
        if self.prev_pos != current_pos and self.map[x,y] < 200:
            self.prev_pos = current_pos
            self.map[x,y] += 1

    def reward_for_research(self):
        x = int(self.info['x']/60)
        y = int(self.info['y']/100)
        if self.map[x,y] >= 1:
            return 2 * (1-self.map[x,y])
        # if self.map[x,y] >= 1:
        #     return -20
        else:
            return 20
    ...
    def press_buttons(self, button_indexes, repeat=10):
        ...
        for i in range(repeat):
            ...
            self.update_map()
            if self.train:
                self.memory.append((self.state, self.action, self.reward, self.next_state, self.done, self.info, self.map))
    ...
    def do(self):
        ...
        if act_value == 1:
            self.press_buttons([7], 4)
        if act_value == 2:
            self.press_buttons([6], 4)
        if act_value == 3:
            self.press_buttons([5], self.INERCIA)
        if act_value == 4:
    ...
    def train(self):
        ...
        while self.total_source_reward < self.TARGET_REWARD and current_try < self.TRY_COUNT:
            ...
            self.max_lives = 3
            current_try += 1
            self.local_reward = deque(maxlen=30)
            self.state = self.env.reset()
            self.done = False
            self.memory.clear()
            # self.map = np.zeros((188, 20),dtype=np.int8)

            for time in range(0, self.TIME_PER_TRY):
                self.action = self.agent.act(self.state, self.action, self.reward, pos_x, self.map)
            ...
            if self.total_source_reward > 0.85 * max_reward:
                max_reward = self.total_source_reward
                print('achieved best experience...')
                # self.view_experience()
                for state, action, reward, next_state, done, info, map in self.memory:
                    self.agent.remember(state, action, reward, next_state, done, info, map)
