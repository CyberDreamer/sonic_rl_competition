...
class CNQAgent:
    def __init__(self, action_size, model_name):
        ...
        self.q_step = 2
        self.q_lenght = 7
        self.info_lenght = self.q_lenght + 2
    ...
    def act(self, state, prev_q_vector, reward, pos_x, map):
        if np.random.rand() <= self.epsilon:
            ...
            map = map.reshape((1, 22, 20, 1))
            q_vector = self.model.predict([state, map])[0]
            # print(q_vector)
        return q_vector
    ...
    def replay(self):
        x_1 = []
        x_2 = []
        leng = len(self.m_state)
        self.q = np.zeros((leng, self.q_lenght))
        
        # place X
        for i in range(leng):
            x_1.append(self.m_state[i])
            x_2.append(self.m_map[i])
        x_1 = np.array(x_1)
        x_2 = np.array(x_2)

        # recalculate Q for all memory
        koko = leng - self.q_history
        # print('koko {} leng {}'.format(koko, leng))
        local_summ = 0
        for i in range(leng, 0):
            state = np.array([self.m_state[i]])
            map = np.array([self.m_map[i]])
            local_q = self.model.predict([state, map])[0]  
            ...
            for j in range(start_history, end_history):
                ...
                map = np.array([self.m_map[i]])
                local_q[m] += j*0.1 * self.gamma * np.amax(self.model.predict([state, map])[0])
